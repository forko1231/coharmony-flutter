...
class _CallScreenState extends State<CallScreen> {
  ...
  void _onConnected(Room? room) {
    if (!mounted) return;
    if (room == null) {
      _callLog('[CALL] screen: connect returned null → closing');
  ...
  void _attachRoom(Room room) {
    ...
    _roomListener = room.createListener()
      ...
      ..on<RoomDisconnectedEvent>((e) {
        _callLog('[CALL] screen: RoomDisconnected reason=${e.reason} → endAndClose');
        _endAndClose();
      });
    ...
    if (!_remoteEverConnected) {
      _ringTimeout = Timer(const Duration(seconds: 45), () {
        if (mounted && !_remoteEverConnected) {
          _callLog('[CALL] screen: ring timeout (45s, no answer) → endAndClose');
          _endAndClose();
        }
      });
    }
        ...
        if (_answered) {
          ...
          _rejectGrace = Timer(const Duration(seconds: 2), () {
            if (mounted && !_answered) {
              _callLog('[CALL] screen: call_rejected stands (no answer) → endAndClose');
              _endAndClose();
            }
          });
        }
  ...
  void _scheduleEndIfRemoteGone() {
    ...
    _remoteGoneTimer = Timer(const Duration(seconds: 6), () {
      ...
      if (_room?.remoteParticipants.isEmpty ?? true) {
        _callLog('[CALL] screen: remote gone for 6s (no call_ended) → endAndClose');
        _endAndClose();
      }
    });
  }
